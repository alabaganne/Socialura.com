...
{
	// Enqueue Google Fonts - Sour Gummy
	wp_enqueue_style('google-fonts-sour-gummy', 'https://fonts.googleapis.com/css2?family=Sour+Gummy:ital,wght@0,100..900;1,100..900&display=swap', array(), null);

	// Check if Tailwind CSS exists and enqueue it
	$tailwind_css = get_template_directory() . '/dist/css/app.css';
	if (file_exists($tailwind_css)) {
		wp_enqueue_style('blankslate-tailwind', get_template_directory_uri() . '/dist/css/app.css', array(), filemtime($tailwind_css));
	}

	// Enqueue theme stylesheet (for WordPress required styles)
	wp_enqueue_style('blankslate-style', get_stylesheet_uri());

	// Check if theme JavaScript exists and enqueue it
	$theme_js = get_template_directory() . '/dist/js/app.js';
	if (file_exists($theme_js)) {
		wp_enqueue_script('blankslate-app', get_template_directory_uri() . '/dist/js/app.js', array('jquery'), filemtime($theme_js), true);
	}
...
add_action('wp_head', 'blankslate_google_fonts_preconnect', 1);
function blankslate_google_fonts_preconnect()
{
?>
